...
def upsert_location(cursor, location):
    cursor.execute("""
        INSERT INTO location_mapping (location_id, country_code, subdivision_code, name, active)
        VALUES (%s, %s, %s, %s, TRUE)
        ON CONFLICT (location_id) DO UPDATE SET
            country_code = EXCLUDED.country_code,
            subdivision_code = EXCLUDED.subdivision_code,
            name = EXCLUDED.name,
            active = TRUE,  -- Reactivate location if it was previously marked inactive
            updated_at = NOW();
        """, (
...
# Function to insert holiday data into the database with versioning and current_version flag
def insert_holiday_data(cursor, location_id, holiday_data, version):
    # Set current_version to FALSE for the current latest version before inserting new data
    cursor.execute("""
        UPDATE holidays SET current_version = FALSE 
        WHERE location_id = %s AND current_version = TRUE;
    """, (location_id,))
    
    for holiday in holiday_data['holidays']:
        cursor.execute("""
            INSERT INTO holidays (location_id, version, date, name, observed, public, current_version)
            VALUES (%s, %s, %s, %s, %s, %s, TRUE)
            ON CONFLICT (location_id, version, date) DO NOTHING;
        """, (
            location_id,
            version,
            holiday['date'],
            holiday.get('name'),  # Use .get to handle missing values
            holiday.get('observed'), 
            holiday.get('public')
        ))

# Function to mark a location as inactive (soft delete)
def deactivate_location(cursor, location_id):
    cursor.execute("""
...
# Function to deactivate locations that are no longer in the current fetch
def deactivate_removed_locations(cursor, current_location_ids):
    cursor.execute("""
        UPDATE location_mapping
        SET active = FALSE, updated_at = NOW()
        WHERE location_id NOT IN %s AND active = TRUE;
    """, (tuple(current_location_ids),))
...
def main():
    ...
    try:
        ...
        # Collect the list of current location IDs
        current_location_ids = [location["location_id"] for location in locations]
        ...
        for location in locations:
            ...
            if holiday_data:
                ...
                cursor.execute("""
                    SELECT COALESCE(MAX(version), 0) + 1 FROM holidays WHERE location_id = %s;
                """, (location_id,))
                version = cursor.fetchone()[0]

                # Insert holiday data
                insert_holiday_data(cursor, location_id, holiday_data, version)
                print("Holiday data inserted successfully!")

        # Deactivate locations that are no longer present in the current fetch
        deactivate_removed_locations(cursor, current_location_ids)
        print("Removed locations deactivated successfully!")
